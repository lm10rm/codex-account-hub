...
直接启动的登录/查询 CLI 由 Windows Job Object 托管，管理器退出时自动回收；允许 CLI 打开的浏览器等后代脱离。恢复时核验 PID、创建时间、程序路径及所属管理器，身份匹配后才结束残留进程。无法核验的旧版纯 PID 记录不会用于自动结束进程。

无法确认子进程退出、加密写回失败或路径包含链接时，会保留文件并提示重试，因此异常情况下临时明文可能继续存在。恢复覆盖本应用的认证事务及已知临时文件；外部进程并发写入、进程启动到纳入 Job Object 的短窗口、真实断电行为仍需进一步加固。不要把应用数据目录同步到云盘或提交到 Git。
